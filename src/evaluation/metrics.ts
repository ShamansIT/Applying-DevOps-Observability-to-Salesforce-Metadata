...
export function toScenarioCsv(results: ScenarioResult[]): string {
  const header = [
    ...
    'final_edge_noise_rate',
    'final_expected_edge_omission_rate',
    'runtime_only_expected',
    'runtime_only_handled',
    'boundary_total',
    'boundary_accuracy',
    'ambiguous_excluded',
    'dist_confirmed',
  ...
  const rows = results.map((result) => {
    ...
    return [
      ...
      m.recall,
      m.f1,
      m.relationshipAccuracy,
      m.orderedPathCoverage,
      m.finalEdgeNoiseRate,
      m.finalExpectedEdgeOmissionRate,
      m.runtimeOnlyExpected,
      m.runtimeOnlyHandled,
      m.boundaryTotal,
      m.boundaryAccuracy,
      m.ambiguousExcluded,
